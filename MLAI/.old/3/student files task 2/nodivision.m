function nodiv = nodivision(Data_spec_MTI2,MD_DopplerAxis,MD_TimeAxis,Te,Diff,Comb1,Comb2,Comb3,Comb4,Comb5,Comb6,OF,TW,PF,filename)
 
Data_spec_MTI2_c = Data_spec_MTI2;
...
saveas(gcf,[pwd,'\',char(filename(1:end-4)),'_MD','_prm_TWL_',num2str(TW),'_OF_',num2str(OF),'_PF_',num2str(PF),'_no_clipping.png'])
save([pwd,'\',char(filename(1:end-4)),'_MD','_prm_TWL_',num2str(TW),'_OF_',num2str(OF),'_PF_',num2str(PF),'_no_clipping.mat'], 'Data_spec_MTI2')
delete(gcf);
...
masked_phase_results_c = masked_phase_features;
else 
masked_phase_results_c = 0;
end
...
masked_unwrapped_results_c = masked_unwrapped_features;
else 
masked_unwrapped_results_c = 0;
end
...
masked_spectrogram_image_results_c = masked_spectrogram_features;
else 
masked_spectrogram_image_results_c = 0;
end
...
Spec_Fea_Result_c = Spec_Feature; 
else 
Spec_Fea_Result_c = 0;
end
Spectrogram = [];
...
Masked_Spec_Fea_Result_c = Masked_Spec_Feature; 
else 
Masked_Spec_Fea_Result_c = 0;
end

nodiv = [mask_results_c masked_phase_results_c masked_unwrapped_results_c masked_spectrogram_image_results_c Spec_Fea_Result_c Masked_Spec_Fea_Result_c];
nodiv = gather(nodiv);
end